...
public class SerialPortUtil {
     ...
     * 打开串口的方法
     */
    public static void openSerialPort(Context context, String port, int baudRate, int dataBits, int stopBits, char parity) {
        Log.i("test", "打开串口");
        Toast.makeText(context, "没有权限22222222222222211", Toast.LENGTH_SHORT).show();
        File file = new File("/dev/" + port);
        Log.i("test", "打开串口112131313123");
        try {
            serialPort = new SerialPort(file, baudRate, dataBits, stopBits, parity, context);
        } catch (Exception e) {
            Log.i("test", "打开串口11213ExceptionExceptionExceptionException1313123");
        }
...
//            byte[] buf = AryChangeManager.hexToByte(data);
            ...
            byte[] zz = data.getBytes();
            outputStream.write(zz);
            outputStream.flush();
            Log.i("test", "串口数据发送成功");
        } catch (IOException e) {
            e.printStackTrace();
